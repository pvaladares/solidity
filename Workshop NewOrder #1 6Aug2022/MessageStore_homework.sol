/* homework for week 2
1) modify the contract so that the ETH paid to buy ownership is paid to the OLD OWNER
2) modify the contract so that the price to buy ownership DOBULES each time
3) CHALLENGE VERSION: the price decreases over time if ownership isn't purchased for a while
*/
...
//the entire contract is contained between the {}
    ...
    /*  Challenge #3 - the price decreases over time if ownership isn't purchased for a while */
    uint public lastBuy;
    ...
    */
    constructor (string memory newData, uint newPrice){
        ...
        /* Challenge #3 - the price decreases over time if ownership isn't purchased for a while */
        lastBuy = 0; // saves the timestamp when a buy is done
    }
    ...
    */
    function buyOwnership() payable public{
        ...
        if(msg.value >= price || msg.value >= updatedPrice()){//msg.value is a special keyword that refers to the amount of ETH sent with the transaction
        
            /* Challenge #2 - ETH paid to buy ownership is paid to the OLD OWNER */
            ...
            /* Challenge #1 - price to buy ownership DOBULES each time it is bought */
            price *= 2; 

            /* Challenge #3 - the price decreases over time if ownership isn't purchased for a while */
            lastBuy = block.timestamp; // saves the timestamp of when the last buy was done


        }else{
            //here we will generate an exception which will "roll back" the transaction including the ETH that was paid
            //note that the transaction fees (gas) are burned though!
            revert("not enough ETH paid for price");
        }
    }

    /* Challenge #3 - the price decreases over time if ownership isn't purchased for a while */
    function updatedPrice() public view returns (uint){

        uint _price;       // new price as function of time
        uint _timeElapsed; // time elapsed since last buy

        _timeElapsed = block.timestamp - lastBuy;
        // price starts to decay, proportionally by hour passed
        // time-window opportunity to buy ownership at discounted price!
        if (lastBuy != 0 && _timeElapsed <= (5 hours))
            _price = price - price * _timeElapsed / (10 hours); // price is cut by half exactly after 5 hours
        else
            _price = price; // after 5 hours the price goes back to previous price
        return _price;
    }
}